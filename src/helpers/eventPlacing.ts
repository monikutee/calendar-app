...
import { compareEventsByStartDate } from "../helpers/layoutHelper";
...
function isNeighbor(eventOne: CalendarEvent, eventTwo: CalendarEvent): boolean {
  ...
  const eventTwoStartsDuringOne =
    startA.getTime() <= startB.getTime() && endA.getTime() > startB.getTime();
  ...
  return (
    eventTwoFinishesDuringOne || eventTwoStartsDuringOne || eventTwoEngulfsOne
  );
}
